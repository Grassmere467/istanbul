   ...
   // }
    ...
    setupListener = function(){
        ...
        io.sockets.on('connection', function (socket) { // First connection
            ...
            console.log('Connectioni from client:  ' + JSON.stringify(address));
...
function runProxyServer(config){
    ...
    connect.createServer(function(req, res, next) {
        ...
        if (config.realtime.config.exclude.indexOf(req.url) > -1){
            console.log('File ' + req.url + ' is flags to exclude');
            proxy.web(req, res);
        }
        else if (endsWith(req.url, '.js')){
            console.log('Have js file: ' + req.url);
           
            request('http://localhost:' + appPort + '/' + req.url, function(error, response, body) {
            ...
            var url = 'http://localhost:' + appPort + '/' + req.url;
            ...
            request(url, function(error, response, body) {
                ...
                if (body.indexOf('<head>') > -1){
                    ...
                    var clientScript = fs.readFileSync(path.resolve(__dirname, '../../client-script.js')).toString();
                    ...
                    res.write('<script>' + socketScript + '</script>');
