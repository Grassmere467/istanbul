...
exports.init = function(config, clientResultsCallback){
    ...
    var clientPostingResultsPort = config.realtime.config.clientPort;
    var Instrumenter = require('./instrumenter');
    var resultsStartUrl = config.realtime.config.resultsStartUrl;
    var clientPostResultsStartUrl = config.realtime.config.clientPostResultsStartUrl;

    if (config.verbose) {
        console.log('Running proxy server on port: ' + proxyPort + ' with app on port: ' + appPort);
        console.log('results will be accessable on: ' + resultsStartUrl);
        console.log('Client will post results to: ' + clientPostResultsStartUrl);
        console.log('Results from client: ' + config.realtime.config.clientResults);
        console.log('Results from server: ' + config.realtime.config.serverResults);
    }
    ...
    connect.createServer(
        function (req, res, next) {
            ...
            if (startsWith(req.url, resultsStartUrl)){
                ...
                return; 
            } else if (config.realtime.config.clientResults === false){
                if (config.verbose) {
                    console.log('Client results disabled so not processing');
                }
            } else if (startsWith(req.url, clientPostResultsStartUrl)){
                if (config.verbose) {
                    console.log('Request is for client post results page: ' + req.url);
                ...
                req.on('end', function(){
                    ...
                    clientResultsCallback(totalData);
                    res.end();
                });

                return; 

            } else if (endsWith(file, '/coverage.js')){
                if (config.verbose) {
                ...
                clientScript = clientScript.replace('__POSTURL__', config.realtime.config.clientPostResultsStartUrl);
                ...
                if (config.verbose) {
                    ...
                    console.log('Will attempt to add coverage to HTML head');
                }
